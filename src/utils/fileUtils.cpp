...
MeshStructure readObjFile(const std::string& name) {
    ...
    std::string inputfile = assetDir + "\\objects\\" + name;
    std::string mtlDir = assetDir + "\\objects\\";
    ...
    for (const auto& shape : shapes) {
        ...
        for (size_t f = 0; f < shape.mesh.num_face_vertices.size(); f++) {
            ...
            for (int v = 0; v < fv; v++) {
                ...
                static std::unordered_map<std::string, unsigned int> vertexMap;
    ...
    MeshStructure meshStructure = {out_vertices, out_indices};
